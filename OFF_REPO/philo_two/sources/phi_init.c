...
/*   Updated: 2020/11/23 18:06:01 by esoulard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			init_tabs(t_phi **phi, sem_t **forks, sem_t **eat_sem)
{
	int		i;
	sem_t	*wr;
	char	*tmp;
	...
	*((*phi)[0].end) = 0;
	...
	while (++i < (*phi)[0].total)
	{
		if (!(tmp = sem_name("/eat", i)))
			return (EXIT_FAILURE);
		sem_unlink(tmp);
		(*eat_sem)[i] = *sem_open(tmp, O_CREAT, 0644, 1);
		free(tmp);
		(*phi)[i].eat_sem = &((*eat_sem)[i]);
